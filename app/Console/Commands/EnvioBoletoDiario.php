...
class EnvioBoletoDiario extends Command
{
    ...
    {
        $this->info('Iniciando comando envioboletodiario');
        Log::info('Iniciando comando envioboletodiario');

        try {
            $hoje = now()->startOfDay();
            $limite = now()->addDays(10)->endOfDay();

            $this->info("Buscando pagamentos com vencimento entre {$hoje} e {$limite}");
            Log::info("Buscando pagamentos com vencimento entre {$hoje} e {$limite}");
            ...
            if ($pagamentos->isEmpty()) {
                $this->warn('Nenhum pagamento encontrado para enviar.');
                return 0;
            }
            ...
            foreach ($pagamentos as $pag) {
                $this->info("Processando pagamento ID {$pag->id} vencimento {$pag->vencimento}");
                Log::info("Processando pagamento ID {$pag->id} vencimento {$pag->vencimento}");
                ...
                try {
                    ...
                    $this->error("Erro ao enviar email para pagamento ID {$pag->id}: {$e->getMessage()}");
                    Log::error("Erro ao enviar email para pagamento ID {$pag->id}: {$e->getMessage()}");
                }
            }

            $this->info("Processamento finalizado. Total de emails enviados: {$enviadosHoje}");
            Log::info("Processamento finalizado. Total de emails enviados: {$enviadosHoje}");
        } catch (\Exception $ex) {
            $this->error("Erro geral no comando: {$ex->getMessage()}");
            Log::error("Erro geral no comando envioboletodiario: {$ex->getMessage()}");
        }

        $this->info('Fim do comando envioboletodiario');
        Log::info('Fim do comando envioboletodiario');
